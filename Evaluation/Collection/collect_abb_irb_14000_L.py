...
def main():
    ...
    for _, theta_arr_i in enumerate(np.array(theta_arr, dtype=np.float64).T):
        ...
        # Store the data.
        theta_desired.append(theta_arr_i); theta_actual.append(ABB_IRB_14000_L_EGM_Cls.Theta)
    ...
    for _, (th_desired_i, th_actual_i) in enumerate(zip(theta_desired, theta_actual)):
        File_IO.Save(f'{project_folder}/Data/ABB_IRB_14000_L/theta_desired', th_desired_i.tolist(), 'txt', ',')
        File_IO.Save(f'{project_folder}/Data/ABB_IRB_14000_L/theta_actual', th_actual_i.tolist(), 'txt', ',')
