...
def main():
    ...
    Description:
        ...
        Note 2:
            The selected robot is the ABB IRB 14000 (right arm), a six-axis articulated robot.
    """
    ...
    ABB_IRB_14000_R_EGM_Cls = Lib.EGM.Core.EGM_Control_Cls('127.0.0.1', 6512)
    ...
    for _, theta_arr_i in enumerate(np.array(theta_arr, dtype=np.float64).T):
        ABB_IRB_14000_R_EGM_Cls.Set_Absolute_Joint_Position(theta_arr_i, True)

        # Store the data.
        theta_desired.append(theta_arr_i); theta_actual.append(ABB_IRB_14000_R_EGM_Cls.Theta)
    ...
    ABB_IRB_14000_R_EGM_Cls.Close()

     # Save the data to a file.
    for _, (th_desired_i, th_actual_i) in enumerate(zip(theta_desired, theta_actual)):
        File_IO.Save(f'{project_folder}/Data/ABB_IRB_14000_R/theta_desired', th_desired_i.tolist(), 'txt', ',')
        File_IO.Save(f'{project_folder}/Data/ABB_IRB_14000_R/theta_actual', th_actual_i.tolist(), 'txt', ',')
